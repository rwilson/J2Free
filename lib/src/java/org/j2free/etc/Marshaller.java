...
public class Marshaller {
    ...
    private final String[] SUPPORTED_DATE_FORMATS = new String[] {
        "yyyy-MM-dd HH:mm:ss",
        "MM-dd-yyyy HH:mm:ss",
    ...
    protected static class Converter {
        ...
        private Converter(Method getter, Method setter) {
            this.getter      = getter;
            this.setter      = setter;
            this.readOnly    = false;
    ...
    public static class MarshalledField {
        ...
        protected MarshalledField(String name, Object value, Converter converter) {
            this.name        = name;
            this.value       = value;
            this.entity      = converter.isEntity();
            this.readOnly    = converter.isReadOnly();
            this.collection  = converter.isCollection();
            this.collectionType = converter.getCollectionType();
        }
     ...
     */
    protected static HashMap<String,Marshaller> marshallers;
    
    static {
        marshallers = new HashMap<String,Marshaller>();
    }
    
    public static Marshaller getForClass(Class klass) {
        
        Marshaller marshaller = marshallers.get(klass.getName());
        ...
        if (marshaller == null) {
            try {
                marshaller = new Marshaller(klass);
            } catch (Exception e) {
    ...
    protected Marshaller(Class klass) throws Exception {
            ...
            if (getter == null && setter == null) {
                ...
                if (converter.isCollection()) {
                    try {
                        ...
                        converter.setCollectionType(collectionType);
                        
                        if (collectionType.getAnnotation(Entity.class) != null)
                            converter.setEntity(true);
                        
                        log.debug(field.getName() + " is entity = " + converter.isEntity());
                        log.debug(field.getName() + " collectionType = " + converter.getCollectionType().getSimpleName());
    ...
    public List<MarshalledField> marshallOut(Object entity, boolean includeTransient) throws IllegalArgumentException {
        ...
        TreeSet<MarshalledField> fields = new TreeSet<MarshalledField>(new Comparator() {
            public int compare(Object one, Object two) {
                if (one.equals(two))
                    return 0;
                
                MarshalledField m0 = (MarshalledField)one;
                MarshalledField m1 = (MarshalledField)two;
        ...
        if (entity == null) {
            for (Map.Entry<Field,Converter> ent : instructions.entrySet()) {
                fields.add(new MarshalledField(ent.getKey().getName(),"",null,false,false,false));
            }
    ...
    public Object marshallIn(Object entity, Map<String,String[]> parameterMap, Controller controller) throws MarshallingException {
        ...
        for (Map.Entry<Field,Converter> ent : instructions.entrySet()) {
            ...
            collectionType = isCollection ? converter.getCollectionType() : null;
            ...
            try {
                ...
                if (field.isAccessible()) {
                    ...
                    if (!isEntity && !isCollection) {
                        ...
                        Marshaller innerMarshaller = Marshaller.getForClass(fieldType);
                        field.set(entity,controller.proxy(fieldType,innerMarshaller.asEntityIdType(newValues[0])));
                        ...
                        if (idsString.length > 0) {
                            ...
                            Marshaller collectionMarshaller = Marshaller.getForClass(collectionType);
                            ...
                            for (String idString : idsString) {
                                ...
                                collection.add(controller.proxy(collectionType,collectionMarshaller.asEntityIdType(idString)));
                            }
                    ...
                    if (setter != null) {
                        ...
                        if (setter.isAccessible()) {
                            if (!isEntity && !isCollection) {
                                ...
                                Marshaller innerMarshaller = Marshaller.getForClass(fieldType);
                                setter.invoke(entity,controller.proxy(fieldType,innerMarshaller.asEntityIdType(newValues[0])));
    ...
    public Object asEntityIdType(String stringId) {
        Class type = getEntityIdType();
