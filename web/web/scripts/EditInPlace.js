...
EditInPlace._saveClick = function(event, id) {
    ...
    if (id_opt['use_dwr']) {
        ...
        var saving = '<span class="' + id_opt['saving_class'] + '">' + id_opt['saving_text'] + '</span>\n';
        ...
        if (id_opt['dwr_params']) {
            for (var i in id_opt['dwr_params']) {
                args.push(id_opt['dwr_params'][i]);
            }
        }

        args.push({
            callback:function(t) { EditInPlace._saveComplete(id,t); },
            errorHandler:function(t) { EditInPlace._saveFailed(id,t); },
            timeout: 5000
        });

        id_opt['dwr_method'].apply({},args);
...
EditInPlace._saveComplete = function(id, t) {
    ...
    if(t.responseText == '') {
        ...
        $(id).innerHTML = id_opt['use_dwr'] ? t : t.responseText;
    }

    // Save the new text as the original text.
    id_opt['orig_text'] = id_opt['use_dwr'] ? t : t.responseText;
...
EditInPlace._formButtons = function(id) {
    ...
    if (id_opt['use_buttons']) {
        return(
                ...
                + id_opt['cancelbutton_text'] + '" /></span>'
        );
    } else {
        return(
                '<span>' +
                '<a id="' + id + '_save"   class="' + id_opt['savebutton_class']   + '">' + id_opt['savebutton_text'] + '</a> | ' +
                '<a id="' + id + '_cancel" class="' + id_opt['cancelbutton_class'] + '">' + id_opt['cancelbutton_text'] + '</a>' +
                '</span></span>'
        );
    }
};